...
The **Cyber Club Portal** is a cutting-edge **service-oriented modular Platform** designed to serve as the central hub for cybersecurity clubs, universities, and technical communities.
...
- **Schema Activation:** Each service is configured to use its own dedicated database schema. The database `search_path` is set to ensure a service only interacts with its own tables, preventing data leakage between services.
...
3.  **Configuration:** Configure your environment variables for **Supabase**, database credentials, and service keys within the `/infra` directory.
4.  **Build Services:** Build the Java services using the Maven Wrapper: `./mvnw clean install` in the respective service directories (e.g., `/identity`, `/portal`).
5.  **Run Locally:** Navigate to the `/infra` directory and launch the full application stack:
6.  **Run Locally:** Navigate to the `/infra` directory and launch the full application stack:
    ```bash
